...
struct LLMConfigCoverageTests {
    ...
    @Test func notificationNameConstant() {
        #expect(Notification.Name.llmConfigDidSave.rawValue == "transcribenote.llmConfigDidSave")
    }

    // MARK: - Keychain Key Edge Cases

    @Test func keychainKeyForEmptyString() {
        #expect(LLMConfig.keychainKey(for: "") == "transcribenote..apiKey")
    }

    @Test func keychainKeyForSpecialCharacters() {
        #expect(LLMConfig.keychainKey(for: "some.key.with.dots") == "transcribenote.some.key.with.dots.apiKey")
    }
}
...
struct LLMModelProfileCoverageTests {
    ...
    @Test func keychainKeyUsesProfileID() {
        let id = UUID()
        let profile = LLMModelProfile(id: id, name: "Test")
        let expected = "transcribenote.profile.\(id.uuidString).apiKey"
...
struct LLMRoleCoverageTests {
    ...
    @Test func subtitleContent() {
        // Verify subtitles are non-empty and unique (locale-independent)
        let subtitles = [LLMRole.live, .overall, .title].map(\.subtitle)
...
struct LLMProfileStoreCoverageTests {
    ...
    private static let suiteName = "com.transcribenote.test.LLMProfileStoreCoverageTests"
