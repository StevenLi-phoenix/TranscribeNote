...
struct LLMModelProfileTests {
    ...
    private static let suiteName = "com.transcribenote.test.LLMModelProfileTests"
    ...
    @Test func keychainKeyFormat() {
        ...
        #expect(profile.keychainKey == "transcribenote.profile.\(id.uuidString).apiKey")
    }
    ...
    @Test func roleDisplayNames() {
        // Verify display names are non-empty and unique (locale-independent)
        let names = [LLMRole.live, .overall, .title].map(\.displayName)
        for name in names {
            #expect(!name.isEmpty)
        }
