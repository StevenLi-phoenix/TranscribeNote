...
struct WelcomeView: View {
    private static let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "TranscribeNote", category: "WelcomeView")
...
private struct WelcomeModelConfigPage: View {
    private static let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "TranscribeNote", category: "WelcomeModelConfig")
    ...
    private func saveQuickConfig() {
        ...
        if !apiKey.isEmpty {
            let keychainKey = "transcribenote.profile.\(profiles[0].id.uuidString).apiKey"
