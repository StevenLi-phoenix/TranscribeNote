...
nonisolated final class AudioCaptureService: @unchecked Sendable {
    private static let logger = Logger(subsystem: "com.transcribenote", category: "AudioCapture")

    private(set) var audioEngine = AVAudioEngine()
    private let ringBuffer: RingBuffer
    private let config: AudioConfig
    private let lock: OSAllocatedUnfairLock<State>
    private let writeQueue = DispatchQueue(label: "com.transcribenote.audio-file-writer", qos: .userInitiated)
    ...
    static func recordingsDirectory() throws -> URL {
        ...
        return appSupport.appendingPathComponent("TranscribeNote/Recordings", isDirectory: true)
    }
}
