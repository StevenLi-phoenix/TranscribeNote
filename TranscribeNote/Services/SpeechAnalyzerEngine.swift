...
nonisolated final class SpeechAnalyzerEngine: @unchecked Sendable {
    private static let log = Logger(subsystem: "com.transcribenote", category: "SpeechAnalyzerEngine")
    ...
    private let queue = DispatchQueue(label: "com.transcribenote.speech-analyzer-engine")
    ...
    init(locale: Locale = .current) throws {
        // Try the requested locale first. If it's not supported by SpeechTranscriber
        // (e.g. zh-Hans when the supported variant is zh_CN), try language-code variants,
        // then fall back to en_US.
        let candidates = Self.localeCandidates(for: locale)
        var resolved: Locale?
        for candidate in candidates {
            ...
            // SpeechTranscriber init succeeds even for unsupported locales on some OS versions,
            // so we accept the first candidate that doesn't crash.
            resolved = candidate
            _ = transcriber // keep the validation reference
            Self.log.info("Engine initialized for locale \(candidate.identifier)")
            break
        }
        guard let finalLocale = resolved else {
            throw EngineError.speechAnalyzerUnavailable
        }
        self.locale = finalLocale
    }

    /// Build a prioritized list of locale candidates: exact → language_REGION → en_US fallback.
    private static func localeCandidates(for locale: Locale) -> [Locale] {
        var candidates = [locale]
        // If locale is e.g. "zh-Hans", try common regional variants
        if let langCode = locale.language.languageCode?.identifier {
            let regionMappings: [String: [String]] = [
                "zh": ["zh_CN", "zh_TW", "zh_HK"],
                "en": ["en_US", "en_GB"],
                "es": ["es_ES", "es_MX", "es_US"],
                "fr": ["fr_FR", "fr_CA"],
                "pt": ["pt_BR"],
                "de": ["de_DE"],
                "ja": ["ja_JP"],
            ...
            if let variants = regionMappings[langCode] {
                candidates.append(contentsOf: variants.map { Locale(identifier: $0) })
            }
        }
        // Always include en_US as ultimate fallback
        if !candidates.contains(where: { $0.identifier == "en_US" }) {
