...
- **`BackgroundSummaryService`**: `@MainActor` singleton that dispatches overall summary + title + tag generation after recording ends, independent of view lifecycle; uses its own `ModelContext` so summaries persist even if user navigates away; `activeTasks: [UUID: Task]` prevents double-dispatch; `awaitAll()` for graceful quit; generates title via separate `LLMRole.title` config after summary completes; generates tags via `PromptBuilder.buildTagPrompt` + `TagParser.parse` after title
- **Auto-tagging**: `TagParser` (`nonisolated enum`) parses LLM tag responses (JSON array, comma-separated, newline-separated); normalizes: trim, dedup case-insensitive, max 5 tags, max 20 chars each; `colorIndex(for:)` uses djb2 hash (deterministic across launches, unlike `String.hashValue`); `TagPillView` renders color-coded capsule pills with 8 predefined colors; `TagRow` shows maxVisible pills with "+N" overflow; `SessionListView` supports tag filtering (AND logic) via `selectedTags` state; tags stored in `RecordingSession.tags: [String]` (already in SchemaV6)
