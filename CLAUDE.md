...
- **`SummaryCardExporter`**: `nonisolated enum` — extracts bullet points/plain text from summary content, renders `SummaryCardTemplate` to `NSImage` via `ImageRenderer` (2x Retina), copies PNG to clipboard or saves via `NSSavePanel`; `SummaryCardData` (nonisolated struct) + `SummaryCardStyle` (.light/.dark/.gradient); render methods are `@MainActor`
- **`SummaryCardTemplate`**: SwiftUI view for image export only (not displayed in app UI) — 600pt wide card with title, date, duration, bullet points or plain text, and "Generated by Notetaker" footer; 3 visual styles
...
- **`notetaker/`** — Main app target
  ...
  - `Views/` — SwiftUI views including `SettingsView` (4-tab layout: `SettingsTab`, `ModelsSettingsTab`, `AboutTab`), `ScheduleView`, `ScheduleEditorView`, `PrivacyDisclosureView`, `SummaryCardView`, `SummaryCardTemplate`, `TranscriptSegmentRow`, `AudioLevelBar`, `ResizeHandle`, `VerticalResizeHandle`, `ChatView`, `SettingsComponents` (reusable settings UI), `ViewModifiers`
