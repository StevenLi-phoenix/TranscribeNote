...
- **`AnalyticsService`**: `nonisolated enum` with pure aggregation functions — `dailyBuckets(sessions:days:now:)` groups `SessionDataPoint`s into `DailyBucket`s with gap-filling for empty days; `summary(sessions:)` computes `AnalyticsSummary` (totals, averages, longest, most active day); `formatHoursMinutes(_:)` for display; no SwiftData dependency
- **Analytics Dashboard**: `AnalyticsView` uses SwiftUI Charts — bar chart (sessions/day), line chart (avg duration trend), summary stat cards; period picker (7/30/90 days); toggled from ContentView toolbar (`chart.bar.xaxis`, Cmd+Shift+A)
...
- **`notetaker/`** — Main app target
  ...
  - `Views/` — SwiftUI views including `SettingsView` (4-tab layout: `SettingsTab`, `ModelsSettingsTab`, `AboutTab`), `ScheduleView`, `ScheduleEditorView`, `PrivacyDisclosureView`, `SummaryCardView`, `TranscriptSegmentRow`, `AudioLevelBar`, `ResizeHandle`, `VerticalResizeHandle`, `ChatView`, `AnalyticsView`, `SettingsComponents` (reusable settings UI), `ViewModifiers`
