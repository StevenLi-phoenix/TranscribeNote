...
- `PromptBuilder` uses shared `styleInstructions(style:task:)` helper for DRY style formatting; `constraintBlock(config:)` enforces no-preamble output and language compliance; `sanitizeLanguage()` strips newlines and limits to 50 chars to prevent prompt injection; `sanitizeInstructions()` limits to 500 chars for guided regeneration; `buildRecipePrompt()` interpolates AI Recipe templates with transcript/title/duration/date variables
- **AI Recipes**: `AIRecipe` (`nonisolated struct: Codable, Sendable, Hashable`) — reusable prompt templates with name, icon (SF Symbol), `promptTemplate` (supports `{{transcript}}`, `{{title}}`, `{{duration}}`, `{{date}}`), `outputSections`, `summaryStyle`, `isBuiltIn`; `RecipeStore` (`nonisolated enum`) stores recipes as JSON in `Application Support/notetaker/recipes.json`; 6 built-in recipes (General Meeting, Sprint Retro, 1-on-1, Sales Call, Brainstorm, Product Review) with stable UUIDs; built-ins cannot be deleted but can be duplicated; `SummarizerService.summarizeWithRecipe()` for recipe-based regeneration; `RecipeListView` in Settings for CRUD; recipe picker Menu in `SummaryCardView` overlay via `onRegenerateWithRecipe` callback
...
- **`notetaker/`** — Main app target
  ...
  - `Models/` — SwiftData models (`RecordingSession`, `TranscriptSegment`, `SummaryBlock`, `ScheduledRecording`), config types (`LLMConfig`, `SummarizerConfig`, `LLMModelProfile`, `VADConfig`, `OverallSummaryMode`, `RepeatRule`, `AIRecipe`), ephemeral types (`ChatMessage`), schema versioning (`Schemas/` V1–V6)
  - `Services/` — Protocol-based engines (`ASREngine`, `LLMEngine`) with multiple implementations (including `FoundationModelsEngine` for Apple Intelligence), `AudioCaptureService`, `AudioPlaybackService`, `AudioExporter`, `SummarizerService`, `BackgroundSummaryService`, `SummaryMarkdownFormatter`, `ChatService`, `PromptBuilder`, `RecipeStore`, `KeychainService`, `CrashLogService`, `SchedulerService`, `CalendarService`
  ...
  - `Views/` — SwiftUI views including `SettingsView` (6-tab layout: Models, LLM, Summarization, Recording, Recipes, About), `RecipeListView`, `ScheduleView`, `ScheduleEditorView`, `PrivacyDisclosureView`, `SummaryCardView`, `TranscriptSegmentRow`, `AudioLevelBar`, `ResizeHandle`, `VerticalResizeHandle`, `ChatView`, `SettingsComponents` (reusable settings UI), `ViewModifiers`
